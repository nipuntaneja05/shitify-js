...
export function shitify(userOptions: ShitifyOptions = {}) {
  ...
  if (options.cursor === 'toilet-paper') {
    document.body.style.cursor = 'url("/public/toilet-paper.png"), auto';
  }

  // 💨 Sound setup
  let fartAudio: HTMLAudioElement | null = null;
  if (options.sound) {
    fartAudio = new Audio("https://cdn.pixabay.com/download/audio/2022/03/15/audio_2f3367e69f.mp3?filename=fart-1-112683.mp3");
